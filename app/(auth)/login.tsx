...
const Login: React.FC = () => {
    ...
    const checkUserExists = async () => {
        try {
            ...
            if (response.status === 200 && response.data) {
                sendPasscode(response.data.id, form.email);
            }
        } catch (error) {
    ...
    const sendPasscode = async (id: string, email: string) => {
        try {
            const response = await axios.post(`${hankoUrl}/passcode/login/initialize`, {user_id: id, email_id: email});
